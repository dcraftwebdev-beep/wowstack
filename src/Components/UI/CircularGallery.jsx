...
class App {
  ...
  createMedias(items, bend = 7, textColor, borderRadius, font) {
    const defaultItems = [
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      // a quick one-stretch auto-scroll when the gallery first appears, so the
      // reel visibly glides across on entry, then settles.
      if (autoScrollOnMount) {
        kick1 = setTimeout(() => {
          if (!app || !app.medias || !app.medias[0]) return;
          app.scroll.target += app.medias[0].width * 5;
          kick2 = setTimeout(() => { if (app) app.onCheck(); }, 1400);
        }, 650);
      }
    });
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, autoScrollOnMount]);
  return (
    <div
